
High-Level Design (HLD) / System Design interviews at companies like Google, Meta, Amazon, Microsoft, Netflix, Uber, Airbnb, Stripe, OpenAI, etc., these are the questions that appear most frequently.



Beginner HLD Interview Questions
Design a URL Shortener (TinyURL/Bit.ly)
Design Pastebin
Design a Parking Lot
Design a Library Management System
Design an ATM
Design an Online Bookstore
Design a Vending Machine
Design a Movie Ticket Booking System
Design an Elevator System
Design a Hotel Booking System
Intermediate System Design Questions
Design WhatsApp
Design Facebook Messenger
Design Instagram
Design Twitter (X)
Design Facebook News Feed
Design LinkedIn
Design Reddit
Design Discord
Design Slack
Design Zoom
Video Streaming
Design YouTube
Design Netflix
Design Twitch
Design Spotify
Design Apple Music

Focus on:

Upload pipeline
Video transcoding
CDN
Recommendations
Live streaming
Search
Search Systems
Design Google Search
Design Elasticsearch
Design Product Search (Amazon)
Design Image Search
Design Autocomplete
Storage Systems
Design Google Drive
Design Dropbox
Design OneDrive
Design AWS S3
Design Distributed File Storage

Topics:

Chunking
Replication
Versioning
Sync
Conflict resolution
Messaging Systems
Design Kafka
Design RabbitMQ
Design Pub/Sub
Design Notification Service
Design Email Service
Payment Systems
Design PayPal
Design Stripe
Design Razorpay
Design UPI
Design Credit Card Payment Gateway

Topics:

Idempotency
Double spending
Transactions
Fraud detection
Ride Sharing
Design Uber
Design Lyft
Design Ola

Topics:

Driver matching
GPS
ETA
Pricing
Food Delivery
Design Swiggy
Design Zomato
Design DoorDash
E-commerce
Design Amazon
Design Flipkart
Design Shopify
Design Inventory System
Cloud Systems
Design AWS EC2
Design Kubernetes Scheduler
Design Docker Registry
Design API Gateway
Design Load Balancer
AI System Design
Design ChatGPT
Design an AI Chatbot
Design an LLM Serving System
Design AI Image Generation
Design Vector Database
Social Media
Design TikTok
Design Snapchat
Design Pinterest
Design Quora
Design Threads
Realtime Systems
Design Google Docs
Design Collaborative Whiteboard
Design Multiplayer Game
Design Live Chat
Design Presence Service
Recommendation Systems
Design YouTube Recommendations
Design Netflix Recommendations
Design Amazon Product Recommendations
Design Friend Recommendation
Design Ads Recommendation
Monitoring Systems
Design Prometheus
Design Grafana
Design Logging System
Design Metrics Collection
Design Alert System
Security Systems
Design Authentication System
Design OAuth Server
Design JWT Authentication
Design Rate Limiter
Design API Key Management
Database Design
Design Distributed Database
Design Redis
Design Cassandra
Design MongoDB
Design MySQL Replication
Networking
Design DNS
Design CDN
Design Reverse Proxy
Design VPN
Design WebSocket Gateway
Advanced FAANG-Level Questions
Design Google Maps
Design Google Photos
Design Gmail
Design Gmail Search
Design Gmail Spam Detection
Design Google Calendar
Design Google Meet
Design Google Docs
Design Google Drive
Design Google Translate
Design Google Analytics
Design Google Ads
Design Google Play Store
Design Apple App Store
Design GitHub
Design GitLab
Design Docker Hub
Design Kubernetes
Design Jenkins
Design CI/CD Pipeline
Design Terraform Cloud
Design Cloudflare
Design Akamai CDN
Design Uber Surge Pricing
Design Airbnb
Design Booking.com
Design Expedia
Design Robinhood
Design Binance
Design Blockchain Explorer
Design NFT Marketplace
Design Healthcare Information System
Design Electronic Health Record (EHR)
Design Food Delivery Platform
Design Logistics System
Design Warehouse Management System
Design IoT Platform
Design Smart Home System
Design Smart City Platform
What Interviewers Expect

For each HLD question, you should be able to discuss:

Functional requirements
Non-functional requirements (availability, scalability, latency, durability)
Capacity estimation
API design
Database schema
High-level architecture
Data flow
Scaling strategies
Caching
Load balancing
Database sharding
Replication
CDN usage
Message queues
Microservices
Fault tolerance
Monitoring and observability
Security
Trade-offs
Bottlenecks and future improvements
A suggested learning order
URL Shortener
Pastebin
Rate Limiter
Notification Service
Tiny Chat/WhatsApp
Twitter/X Feed
YouTube
Uber
Amazon
Google Drive
Netflix
Google Docs
Payment Gateway
Search Engine
Distributed Database
Kubernetes Scheduler
ChatGPT/LLM Serving System

This progression starts with foundational design problems and gradually introduces distributed systems concepts such as caching, partitioning, replication, event-driven architecture, consistency models, and large-scale data processing—the same concepts that underpin many senior-level HLD interviews.